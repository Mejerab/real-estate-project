...
const MultyHouseProperty = () => {
    const multyHouse = useLoaderData();
    const { houseId } = useParams();
    console.log(houseId);
    const aMultyHouse = multyHouse.find(multy => parseInt(houseId) === parseInt(multy.id));
    const { id, name, status, area, location, price, house_facilities, description, house_image } = aMultyHouse;
    let count = 0;
    ...
    return (
        <div>
            ...
            <div className="bg-[#3D52A0] text-white p-6 ml-2 mr-3 mt-6 rounded-2xl">
                <h4 className="text-2xl font-bold text-center">Multy House No.{id}</h4>
            </div>
...
export default MultyHouseProperty;