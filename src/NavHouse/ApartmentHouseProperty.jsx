...
const ApartmentHouseProperty = () => {
    const apartmentHouse = useLoaderData();
    const { houseId } = useParams();
    console.log(houseId);
    const aApartmentHouse = apartmentHouse.find(apartment => parseInt(houseId) === parseInt(apartment.id));
    const { id, name, status, area, location, price, house_facilities,type, description, house_image } = aApartmentHouse;
    let count = 0;
    ...
    return (
        <div>
            ...
            <div className="bg-[#3D52A0] text-white p-6 ml-2 mr-3 mt-6 rounded-2xl">
                <h4 className="text-2xl font-bold text-center">Apartment House No.{id}</h4>
            </div>
            <div className="border w-fit relative rounded-2xl grid grid-cols-2 gap-x-6 ml-2 mr-3 my-6">
                ...
                <div className="flex flex-col justify-center py-10">
                    ...
                    <div className="my-3 flex">
                        <p className="font-semibold mr-6">Type: {type}</p>
...
export default ApartmentHouseProperty;